...
set dist_adj_x 40
set dist_adj_y 40
set x_dim 400; #[expr $dist_adj_x*$num_col] ;#50 ; #[lindex $argv 1]
set y_dim 400; #[expr $dist_adj_y*$num_row] ;#50 ; #[lindex $argv 1]
...
set cbr_pckt_per_sec_11 [expr 10.0*$cbr_pckt_per_sec*$per_11/100]
set cbr_pckt_per_sec_15_4 [expr 10.0*$cbr_pckt_per_sec*(100-$per_11)/100]
...
if {$cbr_pckt_per_sec_11 > 0} { 
	set cbr_interval_11 [expr 10.0/$cbr_pckt_per_sec_11] 
} else {
...
if {$cbr_pckt_per_sec_15_4 > 0} { 
	set cbr_interval_15_4 [expr 10.0/$cbr_pckt_per_sec_15_4] 
} else {
...
set val(idlepower_11) 900e-3			;#Stargate (802.11b) 
set val(rxpower_11) 1560e-3			;#Stargate (802.11b)
set val(txpower_11) 1679e-3			;#Stargate (802.11b)
set val(sleeppower_11) 41.58e-3			;#Stargate (802.11b)
set val(transitionpower_11) 200e-3		;#Stargate (802.11b)	??????????????????????????????/
set val(transitiontime_11) 3			;#Stargate (802.11b)
...
puts "pck_size:$cbr_size data_rate: $data_rate_11 range_11: $range_11"

#CHNG
set num_parallel_flow 0 ;#$num_row ;#[lindex $argv 0]	# along column
set num_cross_flow 0 ;#$num_col ;#[lindex $argv 0]		#along row
set num_random_flow 0 ;#[expr $num_row+$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per random flow
#set num_sink_flow [expr $num_row+$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per sink flow
set num_sink_flow [expr $num_row*$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per sink flow
...
set val(sleeppower_15_4) 0.65e-3		;#LEAP
...
#Phy/WirelessPhy set CSThresh_ $dist(40m)
#Phy/WirelessPhy set RXThresh_ $dist(40m)
#	[$node_([expr $i+$start_15_4]) set netif_(0)] set CSThresh_ $dist(40m)
...
#Phy/WirelessPhy set CSThresh_ $dist(40m)
#Phy/WirelessPhy set RXThresh_ $dist(40m)
#	[$node_($sink_node_15_4) set netif_(0)] set CSThresh_ $dist(40m)
