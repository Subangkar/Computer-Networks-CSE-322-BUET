...
add-packet-header Common Flags AODV DSDV ARP LL MAC CBR IP UDP TCP
...
set num_col 25 ;#[lindex $argv 0] ;#number of column
set dist_adj_x 100
set dist_adj_y 100
...
set num_sink [expr ($num_row * $num_col / 25)]
#set num_sink [lindex $argv 6]
set total_node [expr 2*($num_row * $num_col + $num_sink) ]

set cbr_pckt_per_sec_15_4 [expr $cbr_pckt_per_sec*(100-$per_11)/100]
set cbr_pckt_per_sec_11 [expr $cbr_pckt_per_sec - $cbr_pckt_per_sec_15_4]
...
#############################################################ENERGY PARAMETERS from "End-to-end Routing for Dual-Radio Sensor Networks"

set val(energymodel_11)    EnergyModel     ;
set val(initialenergy_11)  1000            ;# Initial energy in Joules

set val(idlepower_11) 210e-3			;#LEAP (802.11g) 
set val(rxpower_11) 924e-3			;#LEAP (802.11g)
set val(txpower_11) 1320e-3			;#LEAP (802.11g)
set val(sleeppower_11) 25e-3			;#LEAP (802.11g)
set val(transitionpower_11) 176.695e-3		;#LEAP (802.11g)	??????????????????????????????/
set val(transitiontime_11) 3			;#LEAP (802.11g)
...
puts "pck_size:$cbr_size bandwidth: $bandwidth_11 range_11: $range_11 num_BS: $num_sink"

set start_15_4 [expr $num_row*$num_col];#20000
set start_sink [expr 2*$num_row*$num_col];#40000

#CHNG
set num_parallel_flow 0 ;#$num_row ;#[lindex $argv 0]	# along column
set num_cross_flow 0 ;#$num_col ;#[lindex $argv 0]		#along row
set num_random_flow 0 ;#[expr $num_row * $num_col * 50 / 100] ;#[expr $num_row+$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per random flow
set num_sink_flow [expr $num_row * $num_col * 25 / 100] ;#[expr $num_row+$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per sink flow
set num_sink_to_node_flow [expr $num_row * $num_col * 25 / 100] ;#[expr $num_row+$num_col] ;# ;#[expr ($num_row+$num_col)/2] ;# as 2 flows are created per sink #set sink_node 100 ;#sink id, dummy here; updated next
puts "#FLOWS: parallel: $num_parallel_flow cross: $num_cross_flow random: $num_random_flow sink: $num_sink_flow sink_to_node: $num_sink_to_node_flow"
...
set val(netif_15_4) Phy/WirelessPhy ;# network interface type
#set val(mac) Mac/802_11 ;# MAC type
#set val(mac_15_4) Mac/802_15_4 ;# MAC type
set val(mac_15_4) Mac/802_11 ;# MAC type
...
set val(idlepower_15_4) 900e-3			;#Stargate (802.11b) 
set val(rxpower_15_4) 925e-3			;#Stargate (802.11b)
set val(txpower_15_4) 1425e-3			;#Stargate (802.11b)
set val(sleeppower_15_4) 300e-3			;#Stargate (802.11b)
set val(transitionpower_15_4) 176.695e-3	;#Stargate (802.11b)	??????????????????????????????/
set val(transitiontime_15_4) 3			;#Stargate (802.11b)
...
set tr /home/kanchi/a/abmalima/raw_data/simultaneous_tcp.tr
set topo_file /home/kanchi/a/abmalima/raw_data/topo_simultaneous_tcp.txt
...
set tx_rx_mult 0.07075 ;# makes cs range 280m
############################################## st 802.11g [140m, par = 54Mb]

Mac/802_11 set basicRate_ $bandwidth_11
Mac/802_11 set dataRate_ $bandwidth_11
Mac/802_11 set bandwidth_ $bandwidth_11
...
for {set i 0} {$i < [expr $num_row*$num_col]} {incr i} {
	...
	[$node_($i) set netif_(0)] set Frequency_ 2.4e+9
	[$node_($i) set netif_(0)] set channel_number_ $channel_11
	[$node_($i) set netif_(0)] set bandwidth_ $bandwidth_11
	if {$range_11 == 140} { # actually threshold is set for 140m 
		[$node_($i) set netif_(0)] set RXThresh_ 3.26523e-09 ;#1.61163e-08
	}
	[$node_($i) set netif_(0)] set CSThresh_ [expr $tx_rx_mult*[[$node_($i) set netif_(0)] set RXThresh_]]
}

#if {$num_sink_flow > 0} { ;#sink
for {set s 0} {$s < $num_sink} {incr s} {
	set sink_node [expr $start_sink+$s] ;#sink id
	set node_($sink_node) [$ns_ node]
	$node_($sink_node) random-motion 0
...
#	set sink_start_gap [expr 1.0/$num_sink_flow]
	set sink_start_gap 0; #[expr 1.0/$num_sink_flow]

	[$node_($sink_node) set netif_(0)] set Frequency_ 2.4e+9
	[$node_($sink_node) set netif_(0)] set channel_number_ $channel_11
	[$node_($sink_node) set netif_(0)] set bandwidth_ $bandwidth_11
	if {$range_11 == 140} { # actually threshold is set for 140m
		[$node_($sink_node) set netif_(0)] set RXThresh_ 3.26523e-09 ;#1.61163e-08
	}
	[$node_($sink_node) set netif_(0)] set CSThresh_ [expr $tx_rx_mult*[[$node_($sink_node) set netif_(0)] set RXThresh_]]
}

############################################ st 802.11b [140m, 11Mb]
...
for {set i 0} {$i < [expr $num_row*$num_col]} {incr i} {
	...
	$node_([expr $i+$start_15_4]) random-motion 0
	[$node_([expr $i+$start_15_4]) set netif_(0)] set Frequency_ 2.4e+9
...
#	[$node_([expr $i+$start_15_4]) set netif_(0)] set RXThresh_ $dist(40m)
	...
	[$node_([expr $i+$start_15_4]) set netif_(0)] set RXThresh_ 3.52497e-09 ;#1.61163e-08 ;#100 m range
	[$node_([expr $i+$start_15_4]) set netif_(0)] set CSThresh_ [expr $tx_rx_mult*[[$node_([expr $i+$start_15_4]) set netif_(0)] set RXThresh_]]
	[$node_([expr $i+$start_15_4]) set netif_(0)] set bandwidth_ 11e6 ;#1Mbps
}

#if {$num_sink_flow > 0} { ;#sink
for {set s 0} {$s < $num_sink} {incr s} {
	set sink_node_15_4 [expr $start_sink+$num_sink+$s] ;#sink id
	set node_($sink_node_15_4) [$ns_ node]
	$node_($sink_node_15_4) random-motion 0
...
#	set sink_start_gap [expr 1.0/$num_sink_flow]
	...
	[$node_($sink_node_15_4) set netif_(0)] set Frequency_ 2.4e+9
...
#	[$node_($sink_node_15_4) set netif_(0)] set RXThresh_ $dist(40m)
	...
	[$node_($sink_node_15_4) set netif_(0)] set RXThresh_ 3.52497e-09 ;#1.61163e-08  ;#100 m range
	[$node_($sink_node_15_4) set netif_(0)] set CSThresh_ [expr $tx_rx_mult*[[$node_($sink_node_15_4) set netif_(0)] set RXThresh_]]
	[$node_($sink_node_15_4) set netif_(0)] set bandwidth_ 11e6 ;#1Mbps
}
############################################ end
...
for {set s 0} {$s < $num_sink} {incr s} {
	...
	set sink_node [expr $start_sink+$s] ;#sink id
	$node_($sink_node) set X_ $x_pos;#
	$node_($sink_node) set Y_ $y_pos;#
	$node_($sink_node) set Z_ 0.0;#
	puts -nonewline $topofile "SINK NODE $sink_node : at $x_pos $y_pos \n";#

	set sink_node_15_4 [expr $start_sink+$num_sink+$s] ;#sink id
	$node_($sink_node_15_4) set X_ $x_pos;#
	$node_($sink_node_15_4) set Y_ $y_pos;#
	$node_($sink_node_15_4) set Z_ 0.0;#
	puts -nonewline $topofile "SINK NODE $sink_node_15_4 : at $x_pos $y_pos \n";#
}
...
#CHNG
set start_flow_15_4 [expr $num_parallel_flow + $num_cross_flow + $num_random_flow  + $num_sink_flow + $num_sink_to_node_flow];#
for {set i 0} {$i < [expr $num_parallel_flow + $num_cross_flow + $num_random_flow  + $num_sink_flow + $num_sink_to_node_flow]} {incr i} { ;#sink
...
#    set null_($i) [new Agent/Null]
	...
	set f_f_4 [expr $i+$start_flow_15_4]	
	set udp_($f_f_4) [new $tcp_src]
	$udp_($f_f_4) set class_ $f_f_4
	set null_($f_f_4) [new $tcp_sink]
	$udp_($f_f_4) set fid_ $f_f_4
	if { [expr $f_f_4%2] == 0} {
		$ns_ color $f_f_4 Blue
	} else {
		$ns_ color $f_f_4 Red
	}
...
for {set i 0} {$i < $num_parallel_flow } {incr i} {
	...
	$ns_ attach-agent $node_($udp_node) $udp_($f_f_4)
  	$ns_ attach-agent $node_($null_node) $null_($f_f_4)
	puts -nonewline $topofile "PARALLEL: Src: $udp_node Dest: $null_node\n"
} 
...
for {set i 0} {$i < $num_parallel_flow } {incr i} {
     ...
     #set f_4 [expr $i+$start_15_4]	
     set f_f_4 [expr $i+$start_flow_15_4]	
     $ns_ connect $udp_($f_f_4) $null_($f_f_4)
}
#CHNG
for {set i 0} {$i < $num_parallel_flow } {incr i} {
	...
	$cbr_($i) attach-agent $udp_($i)
        ...
        #set f_4 [expr $i+$start_15_4]	
	set f_f_4 [expr $i+$start_flow_15_4]	
	set cbr_($f_f_4) [new Application/Traffic/CBR]
	$cbr_($f_f_4) set packetSize_ $cbr_size
	$cbr_($f_f_4) set rate_ $cbr_rate
	$cbr_($f_f_4) set interval_ $cbr_interval_15_4
	$cbr_($f_f_4) attach-agent $udp_($f_f_4)
} 
...
for {set i 0} {$i < $num_parallel_flow } {incr i} {
	...
	if {$cbr_pckt_per_sec_15_4 > 0} { 
	     #set f_4 [expr $i+$start_15_4]	
	     set f_f_4 [expr $i+$start_flow_15_4]	
	     $ns_ at [expr $start_time+$i*$parallel_start_gap] "$cbr_($f_f_4) start" ;#retain i here
	}
}
####################################CROSS FLOW
#CHNG
...
for {set i 0} {$i < $num_cross_flow } {incr i} {
	...
	set f_f_4 [expr $k+$start_flow_15_4]	
...
for {set i 0} {$i < $num_cross_flow } {incr i} {
	...
	if {$cbr_pckt_per_sec_15_4 > 0} { 
		#set f_4 [expr $k+$start_15_4]	
		set f_f_4 [expr $k+$start_flow_15_4]	
		$ns_ at [expr $start_time+$i*$cross_start_gap] "$cbr_($f_f_4) start"
	}
	incr k
}
#######################################################################RANDOM FLOW
...
for {set i 1} {$i < [expr $num_random_flow+1]} {incr i} {
	...
	set f_f_4 [expr $rt+$start_flow_15_4]	
	$ns_ attach-agent $node_($udp_node) $udp_($f_f_4)
  	$ns_ attach-agent $node_($null_node) $null_($f_f_4)
	puts -nonewline $topofile "RANDOM:  Src: $udp_node Dest: $null_node\n"
	#puts -nonewline "RANDOM:  Src: $udp_node Dest: $null_node\n"
...
for {set i 1} {$i < [expr $num_random_flow+1]} {incr i} {
	...
	if {$cbr_pckt_per_sec_15_4 > 0} { 
		#set f_4 [expr $rt+$start_15_4]	
		set f_f_4 [expr $rt+$start_flow_15_4]	
		$ns_ at [expr $start_time] "$cbr_($f_f_4) start"
	}
	incr rt
}
...
for {set i 1} {$i < [expr $num_sink_flow+1]} {incr i} {
	...
	set sink_node $start_sink
	set d_sink 1000000
	for {set ss $start_sink} {$ss < [expr $start_sink+$num_sink]} {incr ss} {
		set x_n [expr [$node_($udp_node) set X_]/1000.0]
		set y_n [expr [$node_($udp_node) set Y_]/1000.0]
		set x_s [expr [$node_($ss) set X_]/1000.0]
		set y_s [expr [$node_($ss) set Y_]/1000.0]

		if {$d_sink > [expr ($x_n-$x_s)*($x_n-$x_s) + ($y_n-$y_s)*($y_n-$y_s)]} { ;#chooses nearest BS
			set d_sink [expr ($x_n-$x_s)*($x_n-$x_s) + ($y_n-$y_s)*($y_n-$y_s)]
			set sink_node $ss
		}
		#puts "$d_sink"
	}
	#puts "src: $udp_node sink: $sink_node"
	...
	set null_node [expr $sink_node+$num_sink]
...
for {set i 1} {$i < [expr $num_sink_flow+1]} {incr i} {
	...
	if {$cbr_pckt_per_sec_15_4 > 0} { 
		#set f_4 [expr $rt+$start_15_4]	
		set f_f_4 [expr $rt+$start_flow_15_4]	
		$ns_ at [expr $start_time+$i*$sink_start_gap+rand()] "$cbr_($f_f_4) start"
	}
	incr rt
}
...
set rt $r
set num_node [expr $num_row*$num_col]
for {set i 1} {$i < [expr $num_sink_to_node_flow+1]} {incr i} {
	set udp_node [expr int($num_node*rand())] ;#[expr $i-1] ;#[expr int($num_node*rand())] ;# src node
	...
	#	set null_node [expr int($num_node*rand())] ;# dest node
	#}
	$ns_ attach-agent $node_($udp_node) $null_($rt);#$udp_($rt)
  	$ns_ attach-agent $node_($null_node) $udp_($rt);#$null_($rt)
	puts -nonewline $topofile "SINK TO NODE:  Src: $null_node Dest: $udp_node\n"
	...
	#set f_4 [expr $rt+$start_15_4]	
	set f_f_4 [expr $rt+$start_flow_15_4]	
	$ns_ attach-agent $node_($udp_node) $null_($f_f_4);#$udp_($f_f_4)
  	$ns_ attach-agent $node_($null_node) $udp_($f_f_4);#$null_($f_f_4)
	puts -nonewline $topofile "SINK TO NODE:  Src: $null_node Dest: $udp_node\n"
	incr rt
} 
...
for {set ss $start_sink} {$ss < [expr $start_sink+$num_sink]} {incr ss} {
    $ns_ at [expr $start_time+$time_duration] "$node_($ss) reset";
}
...
for {set ss $start_sink} {$ss < [expr $start_sink+$num_sink]} {incr ss} {
	$ns_ initial_node_pos $node_($ss) 4
}
