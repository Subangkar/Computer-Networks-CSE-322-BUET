BEGIN {
	...
	for (i=0; i<max_node; i++) {
		rec_bytes_one_node[i] = 0.0;
		energy_consumption[i] = 0.0;		
	}
...
#		if ( strEvent == "r" ) {
		if ( strEvent == "r" && idPacket >= idLowestPacket) {
			nReceivedPackets += 1 ;		
			nBytes = (nBytes>1000)? 1000:nBytes;
			nReceivedBytes += nBytes;
			rec_bytes_one_node[node] += nBytes;
#			printf("%15.0f\n", nBytes);
...
#	printf( "AverageDelay: %15.5f PacketDeliveryRatio: %10.2f\n", rAverageDelay, rPacketDeliveryRatio ) ;
	printf( "Throughput: %15.2f AverageDelay: %15.5f Throughput per flow: %15.2f\nSent Packets: %15.2f Received Packets: %15.2f Dropped Packets: %15.2f \nPacketDeliveryRatio: %10.2f PacketDropRatio: %10.2f\nTotal time: %10.5f\n", rThroughput, rAverageDelay, rThroughput/num_flow, nSentPackets, nReceivedPackets, nDropPackets, rPacketDeliveryRatio, rPacketDropRatio,rTime) ;
