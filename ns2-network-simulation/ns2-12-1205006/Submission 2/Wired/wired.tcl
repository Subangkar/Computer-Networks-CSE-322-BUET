...
set nodes 20
set flows 12
set cbr_pckt_per_sec 2000
#set cbr_interval 0.0050000000000000001
...
#$argc - nodesber items of arguments passed to a script.
#$argv - list of the arguments.
#$argv0 - name of the script.
#here's the if condition
if {$argc==0} {
        puts "running with default arguments"
} elseif {[lindex $argv 0] == "node"} {
        set nodes [lindex $argv 1] 
        set flows [expr {round(floor($nodes/2))}] 
} elseif {[lindex $argv 0] == "rate"} {
        set cbr_pckt_per_sec [lindex $argv 1]

} elseif {[lindex $argv 0] == "flow"} {
        set flows [lindex $argv 1]
        set nodes [expr $flows*2]
}
